function SolverProcedure( input )
{
	...
	{
		deerTracker[ deer[i].name ] = { activeCounter: 0, restCounter: 0, isResting: false, distance: 0, score: 0 };
	}
	...
	for( var second = 0; second < timeToGo; second++ )
	{
		...
		var winningDeer = 0;
		for( var k = 0; k < deer.length; k++ )
		{
			if( deerTracker[ deer[k].name ].distance > winningDistance )
			{
				winningDistance = deerTracker[ deer[k].name ].distance;
				winningDeer = k;
			}
		}
	...
	for( var i = 0; i < deer.length; i++ )
	{
		if( deerTracker[ deer[i].name ].score > winningScore )
			winningScore = deerTracker[ deer[i].name ].score;
	}


	var output = winningScore;
	return output;
}
